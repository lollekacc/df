...
async function run() {
  ...
  try {
    for (const width of [1440, 390]) {
      ...
      const chats = page.locator('.dealett-chat-navigation > button').first();
      ...
      await page.waitForFunction(() => document.querySelector('.dealett-chat-navigation').textContent.includes('(1)'));
      ...
      assert.equal(await page.getByText('Mitt bredband', { exact: true }).count(), 1);
