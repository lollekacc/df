  ...
  try {
    ...
    for (const width of [1440, 1024, 390]) {
      ...
      for (const route of ['5g-bredband', 'mobilabonnemang', 'familjabonnemang']) {
        ...
        await waitFor(page, `document.querySelectorAll('.offer-card, .bredband-offer-card').length > 0`);
        ...
        const actual = await page.evaluate(`(() => {
          ...
          return {
            ...
            design: {
              ...
              card: styles('.bredband-offer-card, .offer-card', ['width', 'backgroundColor', 'borderRadius', 'boxShadow']),
        ...
        const originalCards = await page.evaluate(`[...document.querySelectorAll('.offer-card')].map((el) => el.textContent)`);
        await page.evaluate(`[...document.querySelectorAll('.operator-filter-button')].find((el) => el.textContent === 'Telia').click()`);
        await waitFor(page, `document.querySelectorAll('.offer-card').length > 0 && [...document.querySelectorAll('.offer-card')].every((el) => el.dataset.operator === 'Telia')`);
        await page.evaluate(`document.querySelector('.operator-filter-button').click()`);
        await waitFor(page, `document.querySelectorAll('.offer-card').length === ${originalCards.length}`);
        assert.deepEqual(await page.evaluate(`[...document.querySelectorAll('.offer-card')].map((el) => el.textContent)`), originalCards);
        if (route === 'familjabonnemang') {
          ...
          await waitFor(page, `document.querySelector('#familySizeValue').textContent === '4' && document.querySelector('.plan-description')?.textContent.startsWith('4 abonnemang')`);
        } else {
          ...
          await page.evaluate(`document.querySelector('.surf-calculator').close()`);
        }
        ...
        assert(await page.evaluate(`!document.querySelector('#rewardSection').classList.contains('is-hidden')`));
        const rewardDesign = await page.evaluate(`['.reward-panel', '.reward-choice', '.reward-summary-bar', '.reward-progress', '.compare-tray'].map((selector) => {
